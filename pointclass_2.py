...
class pointcloud_class(keras.Model):
    ...
    def build(self,inputshape):
        ...
        # self.norm=[layers.LayerNormalization() for i in range(2)]
    ...
    def call(self , points_xyz,training=True):
        ...
        for i in range(self.n):
            ...
            # global_fature=tf.reduce_mean(grouped_feature,axis=-2,keepdims=True)
            # local_fature=grouped_feature-global_fature
            # local_featrue=self.self_attention2[i](local_fature,training=training)
            # # featrue=self.Dense_2[i](tf.concat([local_featrue,tf.squeeze(global_fature,axis=-2)],axis=-1))
            # featrue=self.Dense_2[i](tf.concat([local_featrue,featrue],axis=-1))
            ...
            featrue=self.Dense_[i](tf.concat([local_featrue,tf.squeeze(global_fature,axis=-2)],axis=-1))
